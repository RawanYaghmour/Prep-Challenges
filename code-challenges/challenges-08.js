...
const getEntries = (obj) => {
  ...
  const result = [];
    for (let key in obj) {
        if (obj.hasOwnProperty(key)) {
            result.push(`${key}: ${Array.isArray(obj[key]) ? obj[key].join(',') : obj[key]}`);
        }
    }
    return result;
};
...
const getStudents = (arr) => {
  ...
  const studentInfo = [];
  for (let i = 0; i < arr.length; i++) {
    const studentName = arr[i];
    for (let j = 0; j < courses.length; j++) {
      const course = courses[j];
      if (course.Students.includes(studentName)) {
        studentInfo.push({ Student: studentName, course: course.course });
        break; // Breaks the inner loop once the student is found in a course
      }
    }
  }
